...
public class TCPSocket : NSObject, NSStreamDelegate {
        ...
        if response.isFin && response.bytesLeft <= 0 {
            if response.code == .Ping {
                ...
                dispatch_async(callbackQueue,{
                    ...
                    self.delegate?.TCPSocketDidReceiveMessage(self, text: str! as! String)
                })
                ...
                dispatch_async(callbackQueue,{
                    ...
                    self.delegate?.TCPSocketDidReceiveData(self, data: data)
                })
            }
    ...
    private func errorWithDetail(detail: String, code: UInt16) -> NSError {
        ...
        return NSError(domain: "TCPSocket", code: Int(code), userInfo: details)
    }
